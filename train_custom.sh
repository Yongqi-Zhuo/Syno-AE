...
if [ $MODEL == 'gpt2' ]; then
    ...
    TRAIN_ARGS="--dataset imagenet --imagenet-log-folder $SCRIPT_PATH/logs/$MODEL-$KERNEL_NAME --num-classes 1000 --batch-size 1024 --compile"
fi

mkdir -p $LOGDIR

cd $SCRIPT_PATH/../experiments
python test_run.py --model $MODEL_PROVIDER/$MODEL --kas-test-path $KERNEL_PATH $TRAIN_ARGS > $LOGDIR/$MODEL.log 2>&1