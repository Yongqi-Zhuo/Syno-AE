...
def weights(C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> List[relax.Constant]:
	in_1: relax.Constant = relax.const(np.random.normal(size=(C_out, k_1, k_1 * k_2,)).astype("float32"))
	in_2: relax.Constant = relax.const(np.random.normal(size=(C_in // s, k_2, k_1 * k_2,)).astype("float32"))
	return [in_1, in_2]
...
def build(bb: BlockBuilder, in_0: relax.Expr, in_1: relax.Expr, in_2: relax.Expr, C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> relax.Var:
	...
	def build_subgraph_4(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		ri_1 = te.reduce_axis((0, k_2), "ri_1")
		return te.compute(
			(N, H, H, k_1 * k_2,),
			lambda i_0, i_1, i_2, i_3:
				te.sum(
					te.if_then_else(
						te.all(i_2 + ri_1 - k_2 // 2 >= 0, i_2 + ri_1 - k_2 // 2 < k_2),
						in_0[i_0, ri_0, i_1, te.floormod(i_2 + ri_1 - k_2 // 2 + 1, H)] * in_1[ri_0, ri_1, i_3],
						0.0,
					),
					axis=[ri_0, ri_1],
				),
	...
	def build_subgraph_3(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		ri_1 = te.reduce_axis((0, k_1 * k_2), "ri_1")
		return te.compute(
			(N, C_out, H, H,),
			lambda i_0, i_1, i_2, i_3:
				te.sum(
					te.if_then_else(
						...
						in_0[i_0, i_2 + ri_0 - k_1 // 2, i_3, ri_1] * in_1[i_1, ri_0, ri_1],
						0.0,
					),
					axis=[ri_0, ri_1],
				),
