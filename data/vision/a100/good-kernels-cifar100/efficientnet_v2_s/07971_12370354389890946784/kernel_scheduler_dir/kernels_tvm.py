...
def weights(C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> List[relax.Constant]:
	in_1: relax.Constant = relax.const(np.random.normal(size=(C_out, s, k_1 ** 2 * s,)).astype("float32"))
	in_2: relax.Constant = relax.const(np.random.normal(size=(s, C_in, k_1 ** 2 * s,)).astype("float32"))
	return [in_1, in_2]
...
def build(bb: BlockBuilder, in_0: relax.Expr, in_1: relax.Expr, in_2: relax.Expr, C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> relax.Var:
	...
	def build_subgraph_4(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		return te.compute(
			...
			lambda i_0, i_1, i_2, i_3, i_4:
				te.sum(
					in_0[i_0, ri_0, i_3, i_1] * in_1[i_2, ri_0, i_4],
					axis=[ri_0],
				),
			name="subgraph_4",
		)
	def build_subgraph_3(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		ri_1 = te.reduce_axis((0, k_1 ** 2 * s), "ri_1")
		return te.compute(
			(N, C_out, H, H,),
			lambda i_0, i_1, i_2, i_3:
				te.sum(
					in_0[i_0, te.floordiv(te.floormod(i_2 * s + ri_0 + 1, H * s), s), te.floormod(te.floormod(i_2 * s + ri_0 + 1, H * s), s), i_3, ri_1] * in_1[i_1, ri_0, ri_1],
					axis=[ri_0, ri_1],
				),
			name="subgraph_3",
		)
