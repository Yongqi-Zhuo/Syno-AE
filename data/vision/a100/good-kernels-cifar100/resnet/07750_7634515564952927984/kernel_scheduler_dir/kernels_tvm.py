...
def weights(C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> List[relax.Constant]:
	in_1: relax.Constant = relax.const(np.random.normal(size=(s, C_in // s, k_1, C_out,)).astype("float32"))
	return [in_1]
			...
			lambda i_0, i_1, i_2, i_3:
				te.sum(
					te.if_then_else(
						...
						in_0[i_0, (te.floormod(i_2 * s + ri_1 + 1, H * s) % s) * (C_in // s) + ri_2, te.floormod(i_2 * s + ri_1 + 1, H * s) // s, te.floormod(i_3 + ri_0 - k_1 // 2 + 1, H)] * in_1[ri_1, ri_2, ri_0, i_1],
						0.0,
					),
					axis=[ri_0, ri_1, ri_2],
				),
