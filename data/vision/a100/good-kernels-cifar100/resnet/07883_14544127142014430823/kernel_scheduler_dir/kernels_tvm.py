...
def build(bb: BlockBuilder, in_0: relax.Expr, in_1: relax.Expr, in_2: relax.Expr, C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> relax.Var:
	...
	def build_subgraph_4(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		return te.compute(
			(N, g, H, H, k_1, C_out // g // s,),
			lambda i_0, i_1, i_2, i_3, i_4, i_5:
				te.sum(
					te.if_then_else(
						te.all(i_3 + i_4 - k_1 // 2 >= 0, i_3 + i_4 - k_1 // 2 < k_1),
						in_0[i_0, i_1 * (C_in // g) + ri_0, i_2, i_3 + i_4 - k_1 // 2] * in_1[i_1 * (C_in // g) + ri_0, i_4, i_5],
						0.0,
					),
					axis=[ri_0],
				),
	...
	def build_subgraph_3(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		return te.compute(
			...
			lambda i_0, i_1, i_2, i_3:
				te.sum(
					te.if_then_else(
						te.all(i_2 + ri_2 - k_1 // 2 >= 0, i_2 + ri_2 - k_1 // 2 < k_1),
						in_0[i_0, ri_0, te.floormod(i_2 + ri_2 - k_1 // 2 + 1, H), i_3, ri_1, ri_3] * in_1[ri_0, ri_2, ri_1, i_1, ri_3],
						0.0,
					),
