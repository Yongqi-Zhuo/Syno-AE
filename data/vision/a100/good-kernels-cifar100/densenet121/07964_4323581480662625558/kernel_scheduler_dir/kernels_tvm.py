	...
	def build_subgraph_3(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		return te.compute(
			...
			lambda i_0, i_1, i_2, i_3, i_4:
				te.sum(
					te.if_then_else(
						te.all(i_4 + ri_0 * (C_out // g) + i_2 - C_out ** 3 * k_1 // g ** 3 // 2 >= 0, i_4 + ri_0 * (C_out // g) + i_2 - C_out ** 3 * k_1 // g ** 3 // 2 < C_out ** 3 * k_1 // g ** 3),
						in_0[i_0, ri_1, i_3, te.floormod(i_4 + ri_0 * (C_out // g) + i_2 - C_out ** 3 * k_1 // g ** 3 // 2 + 1, H)] * in_1[i_1, ri_0, ri_1],
						0.0,
					),
					axis=[ri_0, ri_1],
				),
