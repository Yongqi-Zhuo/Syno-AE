...
def weights(C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> List[relax.Constant]:
	in_1: relax.Constant = relax.const(np.random.normal(size=(g, C_in, C_out ** 2 * k_1 // g ** 2,)).astype("float32"))
	return [in_1]
...
def build(bb: BlockBuilder, in_0: relax.Expr, in_1: relax.Expr, C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> relax.Var:
	...
	def build_subgraph_3(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		ri_1 = te.reduce_axis((0, C_out ** 2 * k_1 // g ** 2), "ri_1")
		return te.compute(
			(N, g, H, H,),
			lambda i_0, i_1, i_2, i_3:
				te.sum(
					te.if_then_else(
						te.all(i_3 + ri_1 - C_out ** 2 * k_1 // g ** 2 // 2 >= 0, i_3 + ri_1 - C_out ** 2 * k_1 // g ** 2 // 2 < C_out ** 2 * k_1 // g ** 2),
						in_0[i_0, ri_0, i_2, i_3 + ri_1 - C_out ** 2 * k_1 // g ** 2 // 2] * in_1[i_1, ri_0, ri_1],
						0.0,
					),
					axis=[ri_0, ri_1],
				),
	...
	def build_subgraph_2(in_0: te.Tensor) -> te.Tensor:
		return te.compute(
			...
			lambda i_0, i_1, i_2, i_3:
				te.if_then_else(
					te.all(i_3 + te.floormod(i_1, C_out // g) - C_out // g // 2 >= 0, i_3 + te.floormod(i_1, C_out // g) - C_out // g // 2 < C_out // g),
					in_0[i_0, te.floordiv(i_1, C_out // g), te.floormod(i_2 + 1, H), i_3 + te.floormod(i_1, C_out // g) - C_out // g // 2],
					0.0,
				),
			name="subgraph_2",
		)
