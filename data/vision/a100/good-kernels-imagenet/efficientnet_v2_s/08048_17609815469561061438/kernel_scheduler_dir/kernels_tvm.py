...
def weights(C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> List[relax.Constant]:
	in_1: relax.Constant = relax.const(np.random.normal(size=(C_out // s, k_2, k_1 ** 2,)).astype("float32"))
	in_2: relax.Constant = relax.const(np.random.normal(size=(C_in // s, k_1 ** 2,)).astype("float32"))
	return [in_1, in_2]
	...
	def build_subgraph_5(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		return te.compute(
			...
			lambda i_0, i_1, i_2, i_3, i_4:
				te.sum(
					in_0[i_0, i_1 * (C_in // s) + ri_0, i_2, i_3] * in_1[ri_0, i_4],
					axis=[ri_0],
				),
			name="subgraph_5",
		)
	def build_subgraph_4(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		ri_0 = te.reduce_axis((0, k_2), "ri_0")
		ri_1 = te.reduce_axis((0, k_1 ** 2), "ri_1")
		return te.compute(
			(N, s, C_out // s, H, H,),
			lambda i_0, i_1, i_2, i_3, i_4:
				te.sum(
					te.if_then_else(
						te.all(i_3 + ri_0 - k_2 // 2 >= 0, i_3 + ri_0 - k_2 // 2 < k_2),
						in_0[i_0, i_1, te.floormod(i_3 + ri_0 - k_2 // 2 + 1, H), i_4, ri_1] * in_1[i_2, ri_0, ri_1],
						0.0,
					),
					axis=[ri_0, ri_1],
				),
	...
	def build_subgraph_3(in_0: te.Tensor) -> te.Tensor:
		return te.compute(
			...
			lambda i_0, i_1, i_2, i_3:
				in_0[i_0, te.floordiv(i_1, C_out // s), te.floormod(i_1, C_out // s), i_2, te.floormod(i_3 + 1, H)],
			name="subgraph_3",
		)
