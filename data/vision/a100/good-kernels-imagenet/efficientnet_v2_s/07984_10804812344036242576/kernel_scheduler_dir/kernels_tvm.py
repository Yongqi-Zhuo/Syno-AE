...
def weights(C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> List[relax.Constant]:
	in_1: relax.Constant = relax.const(np.random.normal(size=(C_out, k_1, k_2 * s,)).astype("float32"))
	in_2: relax.Constant = relax.const(np.random.normal(size=(C_in, k_2 * s,)).astype("float32"))
	return [in_1, in_2]
...
def build(bb: BlockBuilder, in_0: relax.Expr, in_1: relax.Expr, in_2: relax.Expr, C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> relax.Var:
	...
	def build_subgraph_5(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		return te.compute(
			...
			lambda i_0, i_1, i_2, i_3:
				te.sum(
					in_0[i_0, ri_0, i_1, i_2] * in_1[ri_0, i_3],
					axis=[ri_0],
				),
			name="subgraph_5",
		)
			...
			lambda i_0, i_1, i_2, i_3, i_4:
				te.sum(
					te.if_then_else(
						...
						in_0[i_0, i_2 * (H // s) + i_3 + ri_0 - k_1 // 2, i_4, ri_1] * in_1[i_1, ri_0, ri_1],
						0.0,
					),
					axis=[ri_0, ri_1],
				),
	...
	def build_subgraph_3(in_0: te.Tensor) -> te.Tensor:
		return te.compute(
			(N, C_out, H, H,),
			lambda i_0, i_1, i_2, i_3:
				in_0[i_0, i_1, te.floordiv(i_2, H // s), te.floormod(i_2, H // s), te.floormod(i_3 + 1, H)],
			name="subgraph_3",
		)
