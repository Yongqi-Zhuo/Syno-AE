...
def weights(C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> List[relax.Constant]:
	in_1: relax.Constant = relax.const(np.random.normal(size=(C_in, k_1, C_out, s,)).astype("float32"))
	return [in_1]
...
def build(bb: BlockBuilder, in_0: relax.Expr, in_1: relax.Expr, C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> relax.Var:
			...
			lambda i_0, i_1, i_2, i_3:
				te.sum(
					te.if_then_else(
						...
						in_0[i_0, ri_0, te.floormod(i_1 * s + ri_1 + 1, H * s) // s, i_2 + ri_2 - k_1 // 2] * in_1[ri_0, ri_2, i_3, te.floormod(i_1 * s + ri_1 + 1, H * s) % s],
						0.0,
					),
					axis=[ri_0, ri_1, ri_2],
				),
	...
	def build_subgraph_2(in_0: te.Tensor) -> te.Tensor:
		return te.compute(
			(N, C_out, H, H,),
			lambda i_0, i_1, i_2, i_3:
				in_0[i_0, te.floormod(i_2 + 1, H), i_3, i_1],
			name="subgraph_2",
		)
