...
def weights(C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> List[relax.Constant]:
	in_1: relax.Constant = relax.const(np.random.normal(size=(k_1, C_in // g // s, g, C_out // g, k_1,)).astype("float32"))
	return [in_1]
	...
	def build_subgraph_3(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		return te.compute(
			...
			lambda i_0, i_1, i_2, i_3, i_4:
				te.sum(
					te.if_then_else(
						te.all(i_1 + ri_0 - k_1 // 2 >= 0, i_1 + ri_0 - k_1 // 2 < k_1, i_4 + ri_2 - k_1 // 2 >= 0, i_4 + ri_2 - k_1 // 2 < k_1),
						in_0[i_0, (i_1 + ri_0 - k_1 // 2) * (C_in // g // s) + ri_1, i_3, te.floormod(i_4 + ri_2 - k_1 // 2 + 1, H)] * in_1[ri_0, ri_1, i_1, i_2, ri_2],
						0.0,
					),
					axis=[ri_0, ri_1, ri_2],
				),
