	...
	def build_subgraph_3(in_0: te.Tensor) -> te.Tensor:
		...
		return te.compute(
			...
			lambda i_0, i_1, i_2, i_3, i_4:
				te.sum(
					te.if_then_else(
						te.all(te.floormod(i_3 * s + ri_0 + 1, H * s) // s + i_4 - k_1 // 2 >= 0, te.floormod(i_3 * s + ri_0 + 1, H * s) // s + i_4 - k_1 // 2 < k_1),
						in_0[i_0, (te.floormod(i_3 * s + ri_0 + 1, H * s) % s) * (C_in // s) + i_1, i_2, te.floormod(te.floormod(i_3 * s + ri_0 + 1, H * s) // s + i_4 - k_1 // 2 + 1, H)],
						0.0,
					),
					axis=[ri_0],
				),
			...
			lambda i_0, i_1, i_2, i_3:
				te.sum(
					te.if_then_else(
						...
						in_0[i_0, ri_2, i_2 + ri_0 - k_1 // 2, i_3, ri_1] * in_1[ri_2, ri_0, ri_1, i_1],
						0.0,
					),
					axis=[ri_0, ri_1, ri_2],
				),
