...
def weights(C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> List[relax.Constant]:
	in_1: relax.Constant = relax.const(np.random.normal(size=(g, k_1, C_out, C_out // g // s,)).astype("float32"))
			...
			lambda i_0, i_1, i_2, i_3:
				te.sum(
					te.if_then_else(
						...
						in_0[i_0, ri_0, te.floormod(i_2 + ri_1 - k_1 // 2 + 1, H), i_3, ri_2] * in_1[ri_0, ri_1, i_1, ri_2],
						0.0,
					),
					axis=[ri_0, ri_1, ri_2],
				),
