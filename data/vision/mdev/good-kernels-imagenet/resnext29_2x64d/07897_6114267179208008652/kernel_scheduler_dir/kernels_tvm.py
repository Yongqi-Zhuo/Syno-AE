...
def weights(C_in: int = -1, C_out: int = -1, H: int = -1, N: int = -1, g: int = 32, k_1: int = 3, k_2: int = 7, s: int = 2, ) -> List[relax.Constant]:
	...
	in_2: relax.Constant = relax.const(np.random.normal(size=(C_out // g, g * s,)).astype("float32"))
	return [in_1, in_2]
	...
	def build_subgraph_5(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		return te.compute(
			...
			lambda i_0, i_1, i_2, i_3, i_4:
				te.sum(
					in_0[i_0, ri_0 * (C_in // g // s) + i_2, i_3, i_4] * in_1[i_1, ri_0],
					axis=[ri_0],
				),
			name="subgraph_5",
		)
	def build_subgraph_4(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
		...
		return te.compute(
			...
			lambda i_0, i_1, i_2, i_3, i_4:
				te.sum(
					te.if_then_else(
						te.all(i_4 + ri_1 - k_1 // 2 >= 0, i_4 + ri_1 - k_1 // 2 < k_1, i_4 + ri_1 - k_1 // 2 + ri_2 - k_1 // 2 >= 0, i_4 + ri_1 - k_1 // 2 + ri_2 - k_1 // 2 < k_1),
						in_0[i_0, i_2, ri_0, i_3, i_4 + ri_1 - k_1 // 2 + ri_2 - k_1 // 2] * in_1[i_1, ri_0, ri_1, ri_2],
						0.0,
					),
					axis=[ri_0, ri_1, ri_2],
				),
